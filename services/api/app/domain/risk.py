...
def evaluate(
    candidate: SignalCandidate, features: FeatureSnapshot, settings: Settings
) -> RiskDecision:
    ...
    final_action = (
        candidate.proposed_action if approved else (Action.HOLD if not failures else Action.AVOID)
    )
